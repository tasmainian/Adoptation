...
public class PetHeapSort {
	...
	private static void sink(ArrayList<petADT> pets, int i, int n) {
		...
		while (2 * i <= n) {
	        ...
	        if (j < n && (pets.get(j-1).getBreedGroup().compareTo(pets.get(j).getBreedGroup()) < 0)) 
	        	j++;
	            
	        if (pets.get(i-1).getBreedGroup().compareTo(pets.get(j-1).getBreedGroup()) >= 0) 
	           	break;
